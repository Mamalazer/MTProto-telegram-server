...
RAND_PART=$(sudo head -c 16 /dev/urandom | sudo xxd -ps -c 256)
...
sudo tee /opt/mtg/config.toml > /dev/null <<EOF
secret = "${SECRET}"
...
# 8. Формируем ссылку
LINK="https://t.me/proxy?server=${IP}&port=3128&secret=${SECRET}"
